...
namespace BesiegeBotsTweaks
{
    ...
    public class InvincibleJointToggle : MonoBehaviour
    {
        ...
        private void Awake()
        {
            Block thisblock = Block.From(base.gameObject);
            ...
            if (thisblock.InternalObject.SimPhysics)
            {
                ...
                if (Player.GetHost() == null || (thisblock.Machine.Player == Player.GetLocalPlayer() ? Player.GetLocalPlayer().IsHost || Player.GetLocalPlayer().InLocalSim : Player.GetLocalPlayer().IsHost && !thisblock.Machine.Player.InLocalSim)) StartCoroutine(MakeInvincible());
